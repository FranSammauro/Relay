...
Fase 4, ver ADR-003 — y no depende de esto en absoluto).
...
- Si Redis se cae, `GET /workers` deja de poder distinguir vivos de
  muertos (todos aparecen como no-vivos, que es el fallo seguro: mejor
  subestimar quién está vivo que sobreestimarlo). Pero la recuperación de
  jobs abandonados (el reaper, ADR-004) sigue funcionando exactamente
  igual, porque corre enteramente sobre `lease_until` en Postgres y nunca
  toca Redis. Esto está probado explícitamente: `tests/recovery.rs` no
  levanta Redis en ningún momento.
- No hace falta un cron ni un job de limpieza para heartbeats viejos — el
  TTL de Redis hace ese trabajo gratis.
- Si el día de mañana se necesita rate limiting o locks de scheduler
  (Fase 5), es la misma instancia de Redis la que lo va a servir — para
  eso se la eligió desde el vamos, no solo para heartbeats.
