...
- **PostgreSQL** persiste el estado completo de cada job, su historial de
  intentos y sus leases (ver `migrations/`). Es la única fuente de verdad
  — si un worker, la API o Redis se caen, ningún job se pierde.
- **API (axum)** expone endpoints HTTP para crear/consultar/cancelar jobs,
  ver su historial de ejecución, el estado agregado de la cola y qué
  workers están vivos ahora mismo.
- **Worker** hace polling de la tabla `jobs` usando
  ...
  `CONCURRENCY`), y además corre en background dos tareas propias:
...
jobs abandonados sigue funcionando igual, porque corre enteramente sobre
PostgreSQL (`lease_until`) y nunca depende de Redis — está probado
explícitamente en `crates/common/tests/recovery.rs`, que no levanta Redis
en ningún momento.
...
retry → retry → dead_letter) y el de recovery
(`crates/common/tests/recovery.rs`, lease vencido → retry/dead_letter)
necesitan un Postgres real — no tiene sentido mockearlos, porque lo que se
está probando es el comportamiento de `SKIP LOCKED` y del backoff/recovery
calculados en SQL bajo condiciones reales. Notablemente, el test de
recovery **no** necesita Redis (ver ADR-002): la recuperación de jobs
abandonados corre enteramente sobre `lease_until` en Postgres. Si no hay
base disponible, los tests se saltan solos con un aviso en vez de romper
la suite. En CI corren siempre (ver `.github/workflows/ci.yml`).

Nota sobre aislamiento: estos tests comparten una sola instancia de
Postgres y están escritos para tolerarlo (marca de `job_type` única por
corrida, tolerancia a jobs reclamados por otro test corriendo en
simultáneo). Si corrés la suite justo después de probar el sistema a mano
(por ejemplo la demo de recovery de más abajo) y te encontrás con un fallo
raro, truncá las tablas y probá de nuevo:
